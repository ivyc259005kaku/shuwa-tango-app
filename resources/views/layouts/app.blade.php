...
<body class="bg-gray-50 text-gray-800 dark:bg-gray-900 dark:text-gray-100 transition-colors duration-200">
    ...
    <header class="bg-blue-100 flex items-center justify-between px-8 py-6 fixed top-0 left-0 right-0 z-50 transition-colors duration-200 relative">
        ...
        <h1 class="text-2xl sm:text-3xl md:text-5xl font-bold text-gray-800 md:absolute md:left-1/2 md:-translate-x-1/2 text-center px-2 whitespace-nowrap">手話単語学習アプリ</h1>
        ...
        <div class="flex items-center gap-3">
            ...
            <form method="POST" action="{{ route('logout') }}">
                @csrf
                <button type="submit" class="border border-gray-400 bg-white text-gray-800 px-2 py-1 rounded text-sm whitespace-nowrap">
                    ログアウト
                </button>
            </form>
        </div>
